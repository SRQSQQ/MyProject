		...
		for (int i = 0; i * XScale < XLength; i++) {
			...
			try {
				canvas.drawText(XLabel[i], XPoint + i * XScale - 10,
						YPoint + 20, paint); // 文字
				// 数据值
				if (i > 0 && YCoord(Integer.parseInt(Data[i - 1])) != -999
						&& YCoord(Integer.parseInt(Data[i])) != -999) // 保证有效数据
					canvas.drawLine(XPoint + (i - 1) * XScale,
							YCoord(Integer.parseInt(Data[i - 1])), XPoint + i * XScale,
							YCoord(Integer.parseInt(Data[i])), paint);
				canvas.drawCircle(XPoint + i * XScale, YCoord(Integer.parseInt(Data[i])), 2,
						paint);
			} catch (Exception e) {
			}
		}
